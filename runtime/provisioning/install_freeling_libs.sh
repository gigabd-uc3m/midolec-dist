...
Options:
  ...
  --skip-check          Do not run scripts/setup_freeling_runtime.sh --check-only.
...
run_runtime_check() {
  ...
  if [[ -x "${REPO_ROOT}/scripts/setup_freeling_runtime.sh" ]]; then
    "${REPO_ROOT}/scripts/setup_freeling_runtime.sh" --check-only
  else
    echo "Runtime check script not found or not executable; skipping."
  fi
}
